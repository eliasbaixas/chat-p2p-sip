...
public class InstantMessagingGUI extends JFrame {
	...
	private static Logger logger = Logger.getLogger(InstantMessagingGUI.class);
	...
	public boolean getAuthorizationForBuddy(String buddy) {
		...
		if (buddyList.hasAuthorization(buddy)) {
			logger
					.debug("DEBUG, InstantMessagingGUI, getAuthorizationForBuddy()"
							+ ", we got a positive authorization from the XML file for: "
							+ buddy);
			return true;
		} else {
			...
			logger
					.debug("DEBUG, InstantMessagingGUI, getAuthorizationForBuddy()"
							+ ", the user has to authorize the buddy to subscribe: "
							+ buddy);
			AlertInstantMessaging alertInstantMessaging = new AlertInstantMessaging(
					"The buddy "
							+ buddy
							+ " wants to add you in his buddy list, do you authorize him?",
	...
	public void start() {
		try {
			...
			logger
					.error(
							"logger, InstantMessagingGUI, start(), Unable to start the UA:",
							e);
	...
	public void stop() {
		try {
			...
			logger
					.error(
							"logger, InstantMessagingGUI, stop(), Unable to stop the UA:",
							e);
		...
		try {
			...
			if (listenerInstantMessaging.tracesViewer != null) {
				try {
					...
					logger.info("logger, restart(), traces viewer closed.");
				} catch (Exception e) {
					logger.error(
							"logger, InstantMessagingGUI, restart(), traces viewer"
									+ " not closed, exception raised:", e);
			...
			logger
					.error(
							"logger, InstantMessagingGUI, restart(), Unable to restart the UA:",
							e);
	...
	public void blockProperties() {
		...
		ConfigurationFrame configurationFrame = listenerInstantMessaging
	...
	public void uploadProperties(String propertiesFile) {
		...
		if (propertiesFile == null) {
			logger
					.warn("logger, Unable to upload the properties: properties file missing");
			return;
		}

		logger
				.info("logger, InstantMessagingGUI, we upload the properties from the "
						+ "file:" + propertiesFile);
		try {
			properties.load(new FileInputStream(propertiesFile));
			logger.info("logger, InstantMessagingGUI, properties uploaded.");
		} catch (Exception e) {
			logger
					.error(
							"logger, InstantMessagingGUI, properties file not found",
							e);
		}
	}
	...
	public void initFields() {
		...
		if (localSipURL != null) {
			...
			logger.debug("logger, InstantMessagingGUI, initFields(),"
		...
		String outboundProxyAddress = properties
				.getProperty("examples.im.outboundProxyAddress");
		if (outboundProxyAddress != null
				&& !outboundProxyAddress.trim().equals(""))
			configurationFrame.outboundProxyAddressTextField
					.setText(outboundProxyAddress);
		String outboundProxyPort = properties
				.getProperty("examples.im.outboundProxyPort");
		if (outboundProxyPort != null && !outboundProxyPort.trim().equals(""))
			configurationFrame.outboundProxyPortTextField
					.setText(outboundProxyPort);

		String registrarAddress = properties
				.getProperty("examples.im.registrarAddress");
		if (registrarAddress != null && !registrarAddress.trim().equals(""))
			configurationFrame.registrarAddressTextField
					.setText(registrarAddress);
		String registrarPort = properties
				.getProperty("examples.im.registrarPort");
		...
		if (outputFile != null && !outputFile.trim().equals("")) {
			...
			Logger logger = Logger.getLogger("gov.nist.sip.instantmessaging");
			try {
		...
		String authenticationFile = properties
				.getProperty("examples.im.authenticationFile");
		if (authenticationFile != null && !authenticationFile.trim().equals(""))
			configurationFrame.authenticationFileTextField
					.setText(authenticationFile);
	...
	public void uploadBuddies() {
		...
		if (xmlBuddiesFile != null) {
			...
			if (buddies != null)
				...
				logger.info("logger, InstantMessagingGUI, uploadBuddies(),"
						+ " No Buddies to upload...");
				buddyList.init(new Vector());
			}
		} else {
			logger.debug("logger, InstantMessagingGUI, uploadBuddies(),"
	...
	public void uploadAuthentication() {
		try {
			...
			if (authenticationFile != null
					&& !authenticationFile.trim().equals("")) {

				XMLAuthenticationParser xmlAuthenticationParser = new XMLAuthenticationParser(
						authenticationFile);
				Vector usersTagList = xmlAuthenticationParser.getUsersTagList();

				authenticationProcess = new AuthenticationProcess(
						instantMessagingUserAgent, usersTagList);
			} else {
				logger
						.error("logger, InstantMessagingGUI, uploadAuthentication(),"
								+ " the file for authentication is missing");
			}
		} catch (Exception e) {
			logger.error("logger, InstantMessagingGUI, uploadAuthentication(),"
					+ " Error trying to upload the authentication file ", e);
	...
	public void initComponents() {
		...
		removeContactMenuItem
				.setToolTipText("Remove a contact from the buddy list");
		...
		offlineJRadioButtonMenuItem
				.setToolTipText("Set your status to offline");
	...
	public void clean() {
		try {
			...
			if (listenerInstantMessaging.rmiregistryProcess != null) {
				...
				logger.debug("logger, rmiregistry process destroyed.");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
