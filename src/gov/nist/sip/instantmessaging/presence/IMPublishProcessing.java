...
public class IMPublishProcessing {
	...
	private static Logger logger = Logger.getLogger(IMPublishProcessing.class);
	...
	public void sendPublish(String localURI, String status) {
		try {
			...
			Address localAddress = addressFactory.createAddress("sip:"
					+ localURI);
			ToHeader toHeader = headerFactory
					.createToHeader(localAddress, null);

			// From header:
			String localTag = Utils.generateTag();
			FromHeader fromHeader = headerFactory.createFromHeader(
					localAddress, localTag);

			// Call-ID:
			CallIdHeader callIdHeader = headerFactory
					.createCallIdHeader(callIdCounter + localURI);
			...
			Request request = messageFactory.createRequest(requestURI,
					"PUBLISH", callIdHeader, cseqHeader, fromHeader, toHeader,
