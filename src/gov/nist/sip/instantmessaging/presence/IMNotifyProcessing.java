...
public class IMNotifyProcessing {
	...
	public static Logger logger = Logger.getLogger(IMNotifyProcessing.class);
	...
	public void processOk(Response response, ClientTransaction clientTransaction) {
		logger.debug("Processing OK received for a NOTIFY ");

		// We have to particular processing to do with the OK..
		logger.debug("OK processed!!!");
	}
	...
	public void processNotify(Request request,
		...
		try {
			...
			logger.debug("Process NOTIFY in progress...");
			 ...
			 * logger.debug("ERROR, processNotify(), PB to retrieve the
			 * dialog"); return; }
			 */
			...
			if ((expiresHeader != null && expiresHeader.getExpires() == 0)
					|| (subscriptionStateHeader != null && subscriptionStateHeader
							.getState().equalsIgnoreCase(
									SubscriptionStateHeader.TERMINATED))) {

				logger
						.debug("DEBUG, IMNotifyProcessing, processNotify(), "
				...
				if (content instanceof String) {
					...
					logger.debug("DEBUG, IMNotifyProcessing, process(), "
				...
				if (text != null && !text.trim().equals("")) {
					...
					try {
						...
						if (xmlType.equals("xpidf+xml")) {
							...
							gov.nist.sip.instantmessaging.presence.pidfparser.PresenceTag presenceTag = xmlPidfParser
									.getPresenceTag();
							if (presenceTag == null)
								logger
										.debug("ERROR: The presence Tag is null!!!");
							else
								logger.debug("the parsed body:"
										+ presenceTag.toString());

							Vector atomTagList = presenceTag.getAtomTagList();
							AtomTag atomTag = (AtomTag) atomTagList
									.firstElement();
							...
							gov.nist.sip.instantmessaging.presence.cpimparser.PresenceTag presenceTag = xmlCpimParser
									.getPresenceTag();
							if (presenceTag == null)
								logger
										.debug("ERROR: The presence Tag is null!!!");
	...
	public void sendNotifyToAllSubscribers(String status, String subStatus) {
		try {
			...
			logger
					.debug("DEBUG, IMNotifyProcessing, sendNotifyToAllSuscribers(),"
							+ " we have to notify our SUBSCRIBERS: let's send a NOTIFY for each one "
							+ "of them (subscribersList: "
							+ subscribersList.size() + ")!!!");
			for (int i = 0; i < subscribersList.size(); i++) {
				Subscriber subscriber = (Subscriber) subscribersList
						.elementAt(i);
				...
				xmlBody = xmlPidfParser.createXMLBody(status, subStatus,
						subscriberName, contactAddress);

				Dialog dialog = subscriber.getDialog();
				if (dialog == null) {
					logger.debug("ERROR, sendNotifyToAllSubscribers(), PB to "
			...
			if (imRegisterProcessing.isRegistered()) {
				...
				javax.swing.JTextField guiSipURI = imUA
						.getInstantMessagingGUI().getLocalSipURLTextField();
	...
	public void sendNotify(Response okSent, String body, Dialog dialog) {
		try {
			...
			if (proxyAddress != null) {
				...
				logger
						.debug("DEBUG, IMNotifyProcessing, sendNotify(), request-uri is null");
				return;
			}
			...
			FromHeader fromHeader = headerFactory.createFromHeader(
					localAddress, dialog.getLocalTag());
			...
			ViaHeader viaHeader = headerFactory.createViaHeader(imUA
					.getIMAddress(), imUA.getIMPort(), imProtocol, branchId);
			...
			if (body == null) {
				...
				ContentTypeHeader contentTypeHeader = headerFactory
						.createContentTypeHeader("application", "xpidf+xml");

				request = messageFactory.createRequest(requestURI, "NOTIFY",
						callIdHeader, cseqHeader, fromHeader, toHeader,
						viaList, maxForwardsHeader, contentTypeHeader, body);
			...
			logger
					.debug("DEBUG, IMNotifyProcessing, sendNotify(), We add the Subscription-State"
			...
			Header header = headerFactory.createHeader("Subscription-State",
					subscriptionState);
			...
			logger.debug("DEBUG, IMNotifyProcessing, sendNotify(),"
					+ " NOTIFY sent:\n");
