...
public class XMLpidfParser extends DefaultHandler {
	...
	private static Logger logger = Logger.getLogger(XMLpidfParser.class);
	...
	public void startDocument() throws SAXException {
		try {
			logger.debug("Parsing XML pidf string");
		} catch (Exception e) {
	...
	public void endDocument() throws SAXException {
		try {
			logger.debug("XML pidf string parsed successfully!!!");
		} catch (Exception e) {
			throw new SAXException("XMLpidfParser error", e);
		}
	}
	...
	public void startElement(String namespaceURI, String lName, // local name
		...
		if (element.compareToIgnoreCase("presentity") == 0) {
			...
			if (uri != null) {
				...
				logger
						.debug("ERROR, XMLpidfParser, the presentity uri is null");
			}
		}
		if (element.compareToIgnoreCase("atom") == 0) {
			...
			if (id != null) {
				...
				logger.debug("ERROR, XMLpidfParser, the atom id is null");
			}
		}
		if (element.compareToIgnoreCase("address") == 0) {
			...
			if (uri != null) {
				...
				logger.debug("ERROR, XMLpidfParser, the address uri is null");
			}
			...
			if (priority != null) {
				...
				logger.debug("DEBUG, XMLpidfParser, the priority is null");
			}
		}
		if (element.compareToIgnoreCase("status") == 0) {
			...
			if (status != null) {
				...
				logger.debug("ERROR, XMLpidfParser, the status status is null");
			}
		...
		if (element.compareToIgnoreCase("msnsubstatus") == 0) {
			...
			if (msnSubStatus != null) {
				...
				logger
						.debug("ERROR, XMLpidfParser, the msnsubstatus substatus is null");
			}
	...
	public String createXMLBody(String status, String subStatus,
			String subscriberName, String contactAddress) {
