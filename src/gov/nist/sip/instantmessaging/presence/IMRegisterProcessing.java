...
public class IMRegisterProcessing {
	...
	private static Logger logger = Logger.getLogger(IMRegisterProcessing.class);
	...
	public void processOK(ResponseEvent responseEvent) {
		...
		JRadioButtonMenuItem offlineJRadioButtonMenuItem = imGUI
				.getOfflineJRadioButtonMenuItem();
		JRadioButtonMenuItem onlineJRadioButtonMenuItem = imGUI
				.getOnlineJRadioButtonMenuItem();
		...
		if (expiresHeader == null || expiresHeader.getExpires() != 0) {
			if (!isRegistered()) {
				logger.debug("process OK for REGISTER (to signIn) in progress");
				// WE are registered!!!!
				...
				signIn = true;
				logger.debug("processing done: signed In");
				...
				imSubscribeProcessing.sendSubscribeToAllPresentities(buddies,
						false);
				...
				logger
						.debug("Dropping OK retransmission for REGISTER, we are already logged in.");
			}
		} else {

			logger.debug("process OK for REGISTER (to signOut) in progress");
			// WE are logged out:
			...
			signIn = false;
			logger.debug("processing done: signed Out");
	...
	public void signIn(String localSipURL) {
		try {
			logger.debug("SignIn in progress for : " + localSipURL);
			...
			String sipDomain = this.imUA.getProxyAddress();
			...
			CallIdHeader callIdHeader = headerFactory
					.createCallIdHeader("nist-sip-im-register-callId"
							+ callIdCounter);
			...
			// server-sided PA
			String methods = Request.INVITE + ", " + Request.SUBSCRIBE + ", "
					+ Request.NOTIFY + ", " + Request.MESSAGE + ", "
	...
	public void signOut(String localSipURL) {
		try {
			logger.debug("process REGISTER (SignOut) in progress for "
					+ localSipURL);
			...
			// Request-URI:
			SipURI requestURI = addressFactory.createSipURI(null, imUA
					.getRegistrarAddress());
			requestURI.setPort(imUA.getRegistrarPort());
