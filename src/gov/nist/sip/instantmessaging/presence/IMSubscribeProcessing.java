...
public class IMSubscribeProcessing {
	...
	private static Logger logger = Logger
			.getLogger(IMSubscribeProcessing.class);
	...
	public void processOK(Response responseCloned,
		...
		try {
			...
			ExpiresHeader expiresHeader = (ExpiresHeader) responseCloned
					.getHeader(ExpiresHeader.NAME);
			if (expiresHeader != null && expiresHeader.getExpires() == 0) {
				logger
						.debug("DEBUG, IMSubscribeProcessing, processOK(), we got"
				...
				if (dialog != null)
					presenceManager.addPresentity(presentityURL,
							responseCloned, dialog);
				else {
					logger
							.debug("ERROR, IMSubscribeProcessing, processOK(), the"
	...
	public void processSubscribe(Request request,
		...
		try {
			...
			InstantMessagingGUI imGUI = imUA.getInstantMessagingGUI();
			boolean authorization = imGUI
					.getAuthorizationForBuddy(presentityURL);
			if (authorization) {
				logger.debug("DEBUG: SubscribeProcessing, processSubscribe(), "
				...
				if (dialog != null)
					presenceManager.addSubscriber(subscriberURL, response,
							dialog);
				else {
					logger
							.debug("ERROR, IMSubscribeProcessing, processSubscribe(), the"
				...
				if (!localStatus.equals("offline")) {
					IMNotifyProcessing imNotifyProcessing = imUA
							.getIMNotifyProcessing();
					Subscriber subscriber = presenceManager
							.getSubscriber(subscriberURL);
					...
					if (subStatus.equals("offline"))
						...
						status = "open";
					String xmlBody = imNotifyProcessing.xmlPidfParser
							.createXMLBody(status, subStatus, subscriberURL,
									contactAddress);
				...
				logger
						.debug("DEBUG, IMSubsribeProcessing, processSubscribe(), "
	...
	public void sendSubscribeToAllPresentities(Vector buddies, boolean EXPIRED) {
		try {
			logger
					.debug("logger, IMSubscribeProcessing, sendSubscribeToAllPresentities(),"
	...
	public void sendSubscribe(String localURL, String buddyURI, boolean EXPIRED) {
		try {
			logger.debug("Sending SUBSCRIBE in progress to the buddy: "
					+ buddyURI);
			...
			if (dialog != null) {
				...
				callIdHeader = (CallIdHeader) headerFactory
						.createCallIdHeader("nist-sip-im-subscribe-callId"
								+ callIdCounter);
			}
			...
			Request request = messageFactory.createRequest(requestURI,
					"SUBSCRIBE", callIdHeader, cseqHeader, fromHeader,
			...
			if (EXPIRED) {
				...
				expiresHeader = headerFactory
						.createExpiresHeader(presenceManager.getExpiresTime());
			}
			request.setHeader(expiresHeader);

			// WE have to add a new Header: "Event"
			Header eventHeader = headerFactory
					.createHeader("Event", "presence");
			request.setHeader(eventHeader);

			// Add Acceptw Header
			Header acceptHeader = headerFactory.createHeader("Accept",
			...
			logger
					.debug("IMSubscribeProcessing, sendSubscribe(), SUBSCRIBE sent:\n"
							+ request);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
