...
public class XMLBuddyParser extends DefaultHandler {
	...
	public void startDocument() throws SAXException {
		try {
			logger.debug("DebugIM, Parsing XML buddies");
		} catch (Exception e) {
			buddies = null;
	...
	public void endDocument() throws SAXException {
		try {
			logger.debug("DebugIM, XML buddies parsed successfully!!!");
		} catch (Exception e) {
			throw new SAXException("XMLBuddyParser error", e);
		}
	}
	...
	public void startElement(String namespaceURI, String lName, // local name
		...
		if (element.compareToIgnoreCase("buddy") == 0) {
			...
			if (uri != null && !uri.trim().equals("") && checkURI(uri))
				...
				logger.debug("DebugIM, the buddy format has to be a sip uri.");
	...
	public static boolean checkURI(String uri) {
		try {
			...
			logger.error("ERROR, DebugIM, the buddy uri has to be a sip uri.",
					e);
			return false;
		}
	}
