...
public class IMUtilities {
	...
	private static Logger logger = Logger.getLogger(IMUtilities.class);
	...
	public static void writeFile(String outFile, String text) {
		try {
			...
			if (file.exists()) {
				...
				logger
						.error("DebugIM, writeFile(),  unable to write, the file:"
								+ outFile + " is missing.");
		} catch (Exception e) {
			logger.error("DebugIM, writeFile(),  unable to write, the file:"
					+ outFile + " is missing.", e);
	...
	public static String getKey(Message message, String header) {
		try {
			...
			if (header.equals("From")) {
				FromHeader fromHeader = (FromHeader) message
			...
			} else if (header.equals("Request-URI")) {
			...
			logger.debug("DEBUG, IMUtilities, getKey(), the key is: "
					+ keyresult);
			return keyresult;
		} catch (Exception e) {
			e.printStackTrace();
