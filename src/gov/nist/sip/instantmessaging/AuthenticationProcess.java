...
public class AuthenticationProcess {
	...
	private static Logger logger = Logger
			.getLogger(AuthenticationProcess.class);
	...
	public AuthenticationProcess(IMUserAgent imUA, Vector usersTagList) {
		...
		if (usersTagList != null)
			this.usersTagList = usersTagList;
		else
			this.usersTagList = new Vector();
	}
	...
	public Header getHeader(Response response) {
		try {
			...
			if (authenticateHeader == null) {
				...
				if (realm == null) {
					logger
							.debug("AuthenticationProcess, getProxyAuthorizationHeader(),"
			...
			digest.initialize(realm, userName, uri, nonce, password, method,
					cnonce, "MD5");

			if (authenticateHeader == null) {
				AuthorizationHeader header = headerFactory
				...
				ProxyAuthorizationHeader header = headerFactory
