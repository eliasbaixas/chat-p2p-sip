...
public class ListenerInstantMessaging {
	...
	private static Logger logger = Logger
			.getLogger(ListenerInstantMessaging.class);
			...
			if (buddy == null || buddy.equals("(empty)")) {
				...
				if (imRegisterProcessing.isRegistered()) {
					if (chatSessionManager.hasAlreadyChatSession(buddy)) {
						// This chat session already exists, we put the focus on
						// it:
						ChatSession chatSession = chatSessionManager
								.getChatSession(buddy);
					...
					new AlertInstantMessaging(
							"You must sign in first to the server!!!",
	...
	public void startRMIregistry() {
		...
		try {
			...
			String rmiregistryClasspath = localRootDirectory + "/classes"
					+ localSeparator + localRootDirectory
					+ "/lib/antlr/antlrall.jar" + localSeparator
					+ localRootDirectory + "/lib/xerces/xerces.jar";

			// The command to execute
			String commandLine = javaHome + "rmiregistry -J-Denv.class.path="
					+ rmiregistryClasspath;

			logger.debug("Starting the rmiregistry:");
			logger.debug(commandLine);
			rmiregistryProcess = runtime.exec(commandLine);

		} catch (Exception e) {
			logger.error("ERROR, starting the rmiregistry, exception raised:",
					e);
	...
	public void tracesViewerActionPerformed(ActionEvent evt) {
		try {
			...
			/*
			 * IMUserAgent imUA=imGUI.getInstantMessagingUserAgent(); String
			 * imAddress=imUA.getIMAddress(); int imPort=imUA.getIMPort(); if
			 * (imAddress==null || imAddress.trim().equals("") || imPort==-1 ) {
			 * logger.debug("ERROR, ListenerInstantMessaging, tracesViewer"+
			 * "ActionPerformed(), you have to set the stack IP address/port"+ "
			 * in the configuration frame."); return; }
			 */
			...
			if (serverLogFile != null) {
				...
				tracesViewer = new TracesViewer(serverLogFile, traces,
						parser.logName, parser.logDescription, parser.auxInfo,
						"IM Traces Viewer", back, faces, actors, logoNist);
				tracesViewer.show();
			} else {
				new AlertInstantMessaging(
						"ERROR: Specify a server log file before viewing the traces!",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			/*
			 * String commandLine=javaHome+"java -classpath
			 * "+getViewerClasspath()+" "+ "tools.tracesviewer.TracesViewer
			 * -stackId "+ proxyIPAddress+":"+proxyPort+" -back "+back+" -faces
			 * "+faces+ " -actors "+actors+" -logoNist "+logoNist;
			 * 
			 * System.out.println(commandLine); Runtime
			 * runtime=Runtime.getRuntime();
			 * viewerProcess=runtime.exec(commandLine); // Thread for Debug:
			 * errorViewerThread=new
			 * ErrorStreamViewerThread(viewerProcess,viewerOutputFrame);
			 * errorViewerThread.start(); inputViewerThread=new
			 * InputStreamViewerThread(viewerProcess,viewerOutputFrame);
	...
	public void addContact(String contact) {
		try {
			...
			IMRegisterProcessing imRegisterProcessing = imUA
					.getIMRegisterProcessing();
			IMSubscribeProcessing imSubscribeProcessing = imUA
					.getIMSubscribeProcessing();
			// if (imRegisterProcessing.isRegistered()) {
			AlertInstantMessaging alert = new AlertInstantMessaging(
					"Do you want to add " + contact + " to your buddy list?",
					AlertInstantMessaging.CONFIRMATION, null);
			if (alert.getConfirmationResult() == JOptionPane.OK_OPTION) {
				// WE have to send a SUBSCRIBE!!!
				imSubscribeProcessing.sendSubscribe(getLocalSipURL(), contact,
						false);
			}
			// }
			// else new AlertInstantMessaging(
			...
			logger
					.error(
							"ERROR, ListenerInstantMessaging, addContact(), buddy is null:",
							e);
	...
	public void addContactActionPerformed(ActionEvent evt) {
		...
		if (imRegisterProcessing.isRegistered()) {
			AlertInstantMessaging alert = new AlertInstantMessaging(
					"Enter the URI" + " of the buddy to add:",
			...
			if (alert.finalInputValue != null) {
				...
				if (buddy == null || !XMLBuddyParser.checkURI(buddy)) {
					new AlertInstantMessaging(
							"The URI is not valid: enter sip:....",
							JOptionPane.ERROR_MESSAGE);
				} else {
					// WE have to check if the contact is not already in the
					// buddy list:
					...
					if (!buddyList.hasBuddy(buddy)) {
						...
						logger
								.debug("DEBUG, ListenerInstantMessaging, "
										+ " addContactActionPerformed(), WE are going to send a SUBSCRIBE...");
						imSubscribeProcessing.sendSubscribe(getLocalSipURL(),
								buddy, false);
					} else {
						logger
								.debug("DEBUG, ListenerInstantMessaging, "
	...
	public void removeContactActionPerformed(ActionEvent evt) {
		...
		if (imRegisterProcessing.isRegistered()) {
			...
			AlertInstantMessaging alertInstantMessaging = new AlertInstantMessaging(
					"Are you sure to remove the selected contact?",
					AlertInstantMessaging.CONFIRMATION, null);
			int confirmationResult = alertInstantMessaging
			...
			if (confirmationResult == JOptionPane.YES_OPTION) {
				...
				if (buddy != null) {
					if (imRegisterProcessing.isRegistered()) {
						...
						logger
								.debug("DEBUG, ListenerInstantMessaging, "
										+ " removeContactActionPerformed(), We have to unSUBSCRIBE to the buddy: "
										+ buddy);
						imSubscribeProcessing.sendSubscribe(getLocalSipURL(),
								buddy, true);
					...
					new AlertInstantMessaging(
							"You must select a contact to remove!",
							JOptionPane.ERROR_MESSAGE);
				}
			}
		} else
	...
	public void onlineActionPerformed(ActionEvent evt) {
		...
		if (imRegisterProcessing.isRegistered()) {
			...
			// of them:
			IMNotifyProcessing imNotifyProcessing = imUA
					.getIMNotifyProcessing();
	...
	public void offlineActionPerformed(ActionEvent evt) {
		...
		if (imRegisterProcessing.isRegistered()) {
			...
			IMNotifyProcessing imNotifyProcessing = imUA
					.getIMNotifyProcessing();
			imNotifyProcessing
					.sendNotifyToAllSubscribers("closed", localStatus);
		} else
			new AlertInstantMessaging(
					"You must sign in first to the server!!!",
					JOptionPane.ERROR_MESSAGE);
	...
	public String getLocalSipURL() {
		try {
			...
			if (text == null || text.trim().equals("")
					|| text.trim().equals("sip:")) {
				new AlertInstantMessaging(
						"You must enter your sip url before signing in!!!",
						JOptionPane.ERROR_MESSAGE);
				return null;
			} else {
	...
	public void signMenuMouseClicked(MouseEvent mouseEvent) {
		logger.debug("Sign in menu mouse clicked.");
		try {
			// IMUserAgent instantMessagingUserAgent =
			// imGUI.getInstantMessagingUserAgent();
			...
			if (localSipURL != null && !localSipURL.trim().equals("")) {
				...
				IMRegisterProcessing imRegisterProcessing = imUA
						.getIMRegisterProcessing();
				IMPublishProcessing imPublishProcessing = imUA
						.getIMPublishProcessing();
				if (imRegisterProcessing.isRegistered()) {
					// It means we will sign out!!!
					imUA.timerStop();
					imPublishProcessing.sendPublish(localSipURL.trim(),
							"offline");
					imRegisterProcessing.signOut(localSipURL.trim());
				} else {
					imRegisterProcessing.signIn(localSipURL.trim());
					// niepin
					// start polling presence of buddies every 1 min
					imUA.timerStart(1, 60000);
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void reSignIn() {
		try {
			...
			if (localSipURL != null && !localSipURL.trim().equals("")) {
				...
				imRegisterProcessing.resetRegister();
				imRegisterProcessing.signIn(localSipURL.trim());
				// niepin
				// start polling presence of buddies every 1 min
				imUA.timerStart(1, 60000);
